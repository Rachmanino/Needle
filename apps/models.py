...
def ResidualBlock(channels, kernel_size, stride=1, device=None, dtype="float32"):
    # assume channels_in = channels_out
    return nn.Residual(
        nn.Sequential(
            nn.ConvBN(channels, channels, kernel_size, stride, device=device, dtype=dtype),
            nn.ConvBN(channels, channels, kernel_size, stride, device=device, dtype=dtype)
        )
    )
...
class ResNet9(ndl.nn.Module):
    def __init__(self, device=None, dtype="float32"):
        ...
        self.model = nn.Sequential(
            ...
            ResidualBlock(32, 3, 1, device=device, dtype=dtype),
            nn.ConvBN(32, 64, 3, 2, device=device, dtype=dtype),
            nn.ConvBN(64, 128, 3, 2, device=device, dtype=dtype),
            ResidualBlock(128, 3, 1, device=device, dtype=dtype),
            nn.Flatten(),
...
class LanguageModel(nn.Module):
    def __init__(self, embedding_size, output_size, hidden_size, num_layers=1,
        ...
        if seq_model == 'rnn':
            ...
            self.model = nn.Transformer(embedding_size, hidden_size, num_layers, device=device, dtype=dtype,
