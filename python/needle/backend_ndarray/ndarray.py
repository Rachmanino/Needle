...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        for i in range(len(self.shape)):
            assert self.shape[i] == 1 or self.shape[i] == new_shape[i]
            if self.shape[i] == 1:
    ...
    def __getitem__(self, idxs):
        ...
        shape = tuple([(idx.stop-idx.start+idx.step-1)//idx.step for idx in idxs])
        strides = tuple([idxs[i].step * self.strides[i] for i in range(len(self.shape))])
    ...
    def pad(self, axes):
        ...
        for i, (size1, size2) in enumerate(axes):
            ...
            new_shape.append(self.shape[i] + size1 + size2)
            slices.append(slice(size1, size1 + self.shape[i]))
        new_shape, slices = tuple(new_shape), tuple(slices)
